...
use crate::{
    kdf::{labeled_extract, HkdfSha256, LabeledExpand}, kem::{Kem as KemTrait, SharedSecret, X448HkdfSha512}, oqs::call_oqs, util::{enforce_equal_len, enforce_outbuf_len, kem_suite_id}, Deserializable, HpkeError, Serializable
};
...
type DilithiumPrivkeyLen = <typenum::U4096 as core::ops::Add<typenum::U800>>::Output;
type DilithiumSignatureLen = <typenum::U4096 as core::ops::Add<typenum::U531>>::Output;
...
const DOMAIN_SEPARATOR_AUTH : &str = "Karolin Varner, Wanja Zaeske, Aaron Kaiser, Sven Friedrich, Alice Bowman, August 2023; From paper: Agile post quantum cryptography in avionics; AKEM (GHP) combiner built from AKEM:HPKE/X448HkdfSha512 + KEM:ML-KEM-1024 + Sig:ML-DSA-87 + KDF:shake256: authenticated";
const DOMAIN_SEPARATOR_NO_AUTH : &str = "Karolin Varner, Wanja Zaeske, Aaron Kaiser, Sven Friedrich, Alice Bowman, August 2023; From paper: Agile post quantum cryptography in avionics; AKEM (GHP) combiner built from AKEM:HPKE/X448HkdfSha512 + KEM:Kyber1024 + Sig:ML-DSA-87 + KDF:shake256: no authentication";
...
impl KemTrait for X448Kyber1024Dilithium {
    ...
    fn derive_keypair(ikm: &[u8]) -> (Self::PrivateKey, Self::PublicKey) {
        ...
        call_oqs(|| unsafe {
            oqs_sys::sig::OQS_SIG_ml_dsa_87_ipd_keypair(
                pkd.as_mut_ptr(),
                skd.as_mut_ptr(),
            )
        }).unwrap();
    ...
    fn encap<R: CryptoRng + RngCore>(
        ...
        if let Some((sk, _pk)) = sender_id_keypair {
            ...
            call_oqs(|| unsafe {
                oqs_sys::sig::OQS_SIG_ml_dsa_87_ipd_sign(
                    sig.as_mut_ptr(),
    ...
    fn decap(
        ...
        if let Some(ref pk) = pk_sender_id {
            call_oqs(|| unsafe {
                oqs_sys::sig::OQS_SIG_ml_dsa_87_ipd_verify(
                    kc.as_ptr(),
                    kc.len(), 
                    sig.as_ptr(),
                    sig.len(),
